...
for p_num_compare, p_num_bs, h_pct in profile_pairs:
    ...
    # ---- Bottom Row: Échelle Diagrams (p-modes only) + HR Diagram ----
    
    # Bottom left: Blue Straggler échelle (all l values, p-modes only)
    ax_ech_bs = axes[1, 0]
    if dnu_bs is not None:
        for l_val in [0, 1, 2]:
            group = groups_bs.get(l_val)
            if group is not None:
                n_pg = np.array(group['n_pg'])
                mask = n_pg > 0  # p-modes only
                if np.any(mask):
                    freqs = np.array(group['freq'].real)[mask]
                    x = freqs % dnu_bs
                    ax_ech_bs.plot(x, freqs, color=l_colors[l_val], marker=l_markers[l_val],
                                  markersize=7, linestyle='None', alpha=0.9,
                                  label=l_labels[l_val])
        ax_ech_bs.set_xlim(0, dnu_bs)
        ax_ech_bs.set_title(rf'Blue Straggler Échelle ($\Delta\nu = {dnu_bs:.2f}$)',
                            fontsize=13, fontweight='bold', pad=10)
    else:
        ax_ech_bs.set_title(r'Blue Straggler Échelle (no $\Delta\nu$)',
                            fontsize=13, fontweight='bold', pad=10)
        ax_ech_bs.text(0.5, 0.5, 'No l=0 p-modes found', transform=ax_ech_bs.transAxes,
                       ha='center', va='center', fontsize=12, color='gray')
    ax_ech_bs.set_xlabel(r'$\nu$ mod $\Delta\nu$ (cyc/day)', fontsize=12)
    ax_ech_bs.set_ylabel(r'Frequency $\nu$ (cyc/day)', fontsize=12)
    ax_ech_bs.legend(fontsize=11, loc='best', framealpha=0.9)
    ax_ech_bs.grid(True, which='both', linestyle=':', color='gray', alpha=0.4)
    ax_ech_bs.tick_params(labelsize=10)
    
    # Bottom middle: Compare star échelle (all l values, p-modes only)
    ax_ech_comp = axes[1, 1]
    if dnu_compare is not None:
        for l_val in [0, 1, 2]:
            group = groups_compare.get(l_val)
            if group is not None:
                n_pg = np.array(group['n_pg'])
                mask = n_pg > 0  # p-modes only
                if np.any(mask):
                    freqs = np.array(group['freq'].real)[mask]
                    x = freqs % dnu_compare
                    ax_ech_comp.plot(x, freqs, color=l_colors[l_val], marker=l_markers[l_val],
                                    markersize=7, linestyle='None', alpha=0.9,
                                    label=l_labels[l_val])
        ax_ech_comp.set_xlim(0, dnu_compare)
        ax_ech_comp.set_title(rf'{mass_compare} M$_\odot$ Échelle ($\Delta\nu = {dnu_compare:.2f}$)',
                              fontsize=13, fontweight='bold', pad=10)
    else:
        ax_ech_comp.set_title(rf'{mass_compare} M$_\odot$ Échelle (no $\Delta\nu$)',
                              fontsize=13, fontweight='bold', pad=10)
        ax_ech_comp.text(0.5, 0.5, 'No l=0 p-modes found', transform=ax_ech_comp.transAxes,
                         ha='center', va='center', fontsize=12, color='gray')
    ax_ech_comp.set_xlabel(r'$\nu$ mod $\Delta\nu$ (cyc/day)', fontsize=12)
    ax_ech_comp.set_ylabel(r'Frequency $\nu$ (cyc/day)', fontsize=12)
    ax_ech_comp.legend(fontsize=11, loc='best', framealpha=0.9)
    ax_ech_comp.grid(True, which='both', linestyle=':', color='gray', alpha=0.4)
    ax_ech_comp.tick_params(labelsize=10)
