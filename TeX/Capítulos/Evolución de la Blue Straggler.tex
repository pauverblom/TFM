...
Se aprecia cómo la Blue Straggler evoluciona a temperaturas superiores que estrellas similares en masa (como es el caso de la estrella de $1.82 M_\odot$), fruto de su estructura interior rejuvenecida (Figura \ref{fig:evolution_graphs}).
...
Resulta interesante comparar el interior de la Blue Straggler con el de una estrella de masa similar. Esto puede verse con un diagrama de Kippenhahn (Figura \ref{fig:kippenhahn_comparison}).
...
\begin{figure}[H]
  ...
  \caption{Frecuencias individuales de los modos $\nu^{\text{Ref}}_{n_{pg}, l} - \nu^{\text{BS}}_{n_{pg}, l}$ en función del orden radial $n_{pg}$ para los modos $l=0, 1, 2$. Los paneles se encuentran ordenados de mayor a menor fracción de hidrógeno central (con los mismos puntos que los denotados en la Figura \ref{fig:global_hr_diagram}).}
...
Las diferencias máximas observadas en las frecuencias individuales para cada modo, restringidas a $n_{pg} \in [-10, 10]$, se cuantifican en la Tabla \ref{tab:max_diff_indfreq}.
...
\begin{table}[htpb]
  ...
  \begin{tabular}{
    ...
    \toprule
    {Par} & {$H_c$ (\%)} & {$|\Delta(\delta\nu_{02})|_\mathrm{max}$ (ciclos\,día$^{-1}$)} \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption{Diferencia máxima absoluta en la pequeña separación $|\Delta(\delta\nu_{02})|_\mathrm{max}$ entre la Blue Straggler y la estrella de evolución estándar, para cada par de comparación. Los pares están emparejados a idéntica fracción de hidrógeno central $H_c$, de modo que las diferencias son de carácter puramente estructural.}
